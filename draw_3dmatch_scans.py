...
def draw_scans(src, tgt, image_name, point_size=3, view=False):
...
def draw_correspondences(src, src_orig, tgt, tgt_orig, weights, attn, image_name, point_size=3, k=256, view=False):
    pcd1 = o3d.geometry.PointCloud()
    pcd1.points = o3d.utility.Vector3dVector(src)
...
# src_true = (scan['Rtrue'] @ scan['src_quant'].T).T + scan['Ttrue'].T
# draw_scans(scan['src_quant'], scan['tgt_quant'], image_name="test", point_size=3)
# draw_scans(src_true, scan['tgt_quant'], image_name="test", point_size=3)
#
# src_true = (scan['Rtrue'] @ scan['src'].T).T + scan['Ttrue'].T
# draw_scans(scan['src'], scan['tgt'], image_name="test", point_size=3)
# draw_scans(src_true, scan['tgt'], image_name="test", point_size=3)
# draw_scans(scan['src_orig'], scan['tgt_orig'], image_name="test", point_size=3 )
