...
def extract_data(path):
    ...
    # ['src', 'tgt', 'Rtrue', 'Ttrue', 'Rest', 'Test', 'corres_pts_for_src', 'log_attn_row', 'w_src']
...
def draw_scans(src, tgt, image_name, save_dir, point_size=3, view=False):
    ...
    vis.get_render_option().load_from_json("o3d_config/modelnet.json")
    ...
    param = o3d.io.read_pinhole_camera_parameters("o3d_config/modelnet_camera.json")
    ...
    o3d.io.write_pinhole_camera_parameters('o3d_config/modelnet_camera.json', param)
...
def draw_correspondences(src, tgt, weights, corr_idx, image_name, save_dir, point_size=3, k=256,
    ...
    pcd2 = o3d.geometry.PointCloud()
    # print(tgt.shape)
    tgt += np.array([2, 0, 0]).reshape(1, 3)
    ...
    param = o3d.io.read_pinhole_camera_parameters("o3d_config/modelnet_camera_corr.json")
    ctr.convert_from_pinhole_camera_parameters(param)
    vis.get_view_control().convert_from_pinhole_camera_parameters(param)
    ...
    param = ctr.convert_to_pinhole_camera_parameters()
    o3d.io.write_pinhole_camera_parameters('o3d_config/modelnet_camera_corr.json', param)
...
for path in paths:
    ...
    draw_scans(scan['src'], scan['tgt'], image_name= head + "_init", save_dir=save_dir, point_size=3, view=True)
    draw_scans(src_true, scan['tgt'], image_name=head + "_gt", point_size=3, save_dir=save_dir, view=True)
    draw_scans(src_est, scan['tgt'], image_name=head + "_est", point_size=3, save_dir=save_dir, view=True)
    # break
    # src_est = (scan['Rest'] @ scan['src'].T).T + scan['Test'].T
